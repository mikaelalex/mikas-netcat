...
#	  or
#
#	  $ docker run -ti --rm -p 8182:8182 chilcano/netcat:jessie -vvl -p 8182		// starts container and runs netcat listening on a TCP port
#	  $ docker run -ti --rm -p 9192:9192/udp chilcano/netcat:jessie -vvul -p 9192	// starts container and runs netcat listening on an UDP port
#
...
COPY ep_nc.sh /
COPY script.sh / 
RUN chmod a+x /ep_nc.sh ; chmod a+x /script.sh
ENTRYPOINT ["/ep_nc.sh"]
